...
class ImportLemburSuperController extends Controller
{
    public function index(Request $request)
    {
        $type = $request->type;
        $slips = ApprovalLembur::where('tipe_karyawan', $type)->get();
        return view('superuser.import_lembur', compact('slips'));
    }
    ...
    public function import(Request $request)
    {
        ...
        $file->move('importLemburSuperDoc', $nama_file);

        // Excel::import(new KaryawanTetapImport, public_path('/importTetap/' . $nama_file));
        $collection = Excel::toCollection(new KaryawanTetapImport, public_path('/importLemburSuperDoc/' . $nama_file));
